...
def clinical_predict(meta: pd.DataFrame, labels: pd.Series):
    ...
    for variable_name in TO_PREDICT:
        ...
        accuracies.loc[variable_name], _ = predict_clinical(
            variable,
            _labels
        )
...
def makeFigure():
    ...
    patient_factors[:] = scale(patient_factors)
    ...
    cyto_factors = pd.DataFrame(
        tpls.Xs_factors[0][2] * -1, index=data.Cytokine.values, columns=[1, 2]
    )
    ...
    yticks = [round(i, 2) for i in np.arange(-1, 1.05, 0.2)]
    ax.set_yticks(yticks)
    ax.set_yticklabels(yticks)
