...
def makeFigure():
    ...
    for index, cytokine_tp in enumerate(
        ...
        cytokines = (
            cytokine_measurements.loc[{"Cytokine Timepoint": cytokine_tp}]
            .squeeze()
            .to_pandas()
        )
        ...
        il_1ra_corr = pearsonr(
            il_1ra.loc[:, "IL-1RA"], il_1ra.loc[:, "Eotaxin"]
        )
        il_15_corr = pearsonr(il_15.loc[:, "IL-15"], il_15.loc[:, "Eotaxin"])

        ax.bar(index * 3, il_1ra_corr.statistic, width=1, color="tab:blue")
        ax.bar(index * 3 + 1, il_15_corr.statistic, width=1, color="tab:orange")
    ...
    ax.set_xticks(
        np.arange(
            0.5, len(cytokine_measurements["Cytokine Timepoint"].values) * 3, 3
        )
    )
    ...
    eotaxin_il15 = (
        cytokine_measurements.loc[
            {"Cytokine": ["IL-15", "Eotaxin"], "Cytokine Timepoint": "LF"}
        ]
        .squeeze()
        .to_pandas()
        .dropna()
    ...
    plot_scatter(eotaxin_il15, ax)

    ax = axs[2]

    eotaxin_il1ra = (
        cytokine_measurements.loc[
            {"Cytokine": ["IL-1RA", "Eotaxin"], "Cytokine Timepoint": "LF"}
        ]
        .squeeze()
        .to_pandas()
        .dropna()
